...
### WS-M4 — Quality & perf (P2 — scoped, one item built, rest deferred/skipped)
A scoping pass (multi-agent: empirical RD/profile measurement + per-item tractability) re-grounded this workstream post-pivot. Outcome: **one high-value, byte-safe item built; the perceptual-quality items honestly deferred or skipped** (perceptual parity is low value for a lossless-archival medical use, and the jpegli "field oracle" needs an instrumented libjxl build).
- [x] **WS-D float32 decode output** — `.float32` emits **raw reconstructed sample values** (not normalized; asymmetric to the encoder's [0,1] input, documented) for grayscale + XYB; YCbCr/lossless reject it cleanly. Zero-rounding passthrough → preserves sub-integer precision the uint path discards; also fixed a latent `bufferSizeMismatch` bug. Decode-only (no encode byte-identity risk). `Float32DecodeTests`.
- [ ] *(deferred, build-later, P3)* **WS-C perf** — the profiler shows the biggest remaining decode cost is the (already-vDSP'd, memory-bound) inverse color convert; the best *byte-identical* target is **Step-7 reconstruction per block-range** (`JLIDecoder.swift:224-388`, blocked on the 4 shared scratch buffers → needs per-worker slices, copying the encoder's proven `@unchecked Sendable`+`concurrentPerform` template). **Low medical value** (helps large lossy color, not the lossless/grayscale clinical path); if pursued, gate on a parallel-vs-serial decode-output SHA-256 in `RegressionCorpusTests`. Fused dequant+IDCT is identity-risky → last.
- [ ] *(deferred, P3 spike)* **jpegli MR gap** — *not* oracle-blocked to *change*: the AQ field is computed on gamma-domain luma (`JLIEncoder.swift:324-327`) where libjxl uses opsin-transformed luminance (a double-gamma mismatch) — a principled ~1-day opsin-input spike, RD-validatable via `JLIBench --rd-matrix` (no instrumented libjxl needed). **But** it's upside on the opt-in, non-default lossy `jpegliAdaptiveQuant` path; the empirical pass also found **no large systematic MR gap** in the available corpus (≤10%, image-dependent) and **no `mr` cache entry to remeasure**. Low priority.
- [ ] *(env-blocked, P3)* **DocC catalog** — small (~2–4h) but blocked here by `git safe.bareRepository=explicit` (SwiftPM can't fetch swift-docc-plugin); do it in an unrestricted env/Xcode. No medical-evidence value.
- [x] ~~**WS-E XYB re-eval**~~ — **skipped.** Settled at ≈parity, and XYB is a 3-channel *color* space — inert on the all-grayscale medical corpus. Zero medical value.
- [x] ~~**jpegli field oracle**~~ (original WS-M4 headline) — **skipped.** `cjpegli` is installed so the RD-matrix jpegli rows are real, but the *instrumented* `quant_field` dump needs patching libjxl-from-source; post-pivot the RD harness already answers the only question that matters (did quality improve?), so the oracle isn't needed.

### Sequencing & gate
**WS-M1 ✅ + WS-M2 ✅ + WS-M3 ✅ + WS-M4 (float32 built; rest deferred/skipped) ✅ landed** — the full 0.4.0 "medical foundations" scope is complete (220 tests / 26 suites green). **`v0.4.0` is ready to tag** (would bump the version from 0.3.0). The remaining WS-M4 items are P3 (perf, jpegli spike, DocC) and explicitly deferred/skipped with reasons above. Discipline unchanged: bit-exact gates, fuzz throws, CI-green, every claim corpus-measured.
